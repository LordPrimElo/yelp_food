...
router.post("/", isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    req.flash(
      "success",
      `Food Item Added - ${newFoodItem.title} created! Nice job!`
    );
...
router.post("/vote", isLoggedIn, async (req, res) => {
  ...
  const votingError = () => {
    req.flash(
      "error",
      "Please try voting again! We're sorry for the inconvenience :("
    );
    res.redirect("/foods");
...
router.get("/saved", isLoggedIn, async (req, res) => {
  ...
  const savedFoods = await foodItem
    .find({ savedByUsers: req.user.username })
    .exec();
...
router.put("/:id/save", isLoggedIn, async (req, res) => {
  ...
  if (isSavedByUser === -1) {
    ...
    await foodItem
      .findByIdAndUpdate(req.params.id, newSaved, { new: true })
      .exec();
...
router.get("/genre/:genre", async (req, res) => {
  try {
    ...
    if (validGenres.includes(req.params.genre.toLowerCase())) {
      const foodstuffs = await foodItem
...
router.put("/:id", isLoggedIn, isFoodOwner, async (req, res) => {
  ...
  try {
    await foodItem
      .findByIdAndUpdate(req.params.id, updatedNewFoodItem, { new: true })
      .exec();
...
router.delete("/:id", isLoggedIn, isFoodOwner, async (req, res) => {
  try {
    const deletedFoodItem = await foodItem
      .findByIdAndDelete(req.params.id)
      .exec();
    req.flash(
      "success",
      "You have become Death, the destroyer of food items..."
    );
    res.redirect("/foods");
  } catch (err) {
